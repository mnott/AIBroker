...
**A finished run releases its own claim** — after completing the task, not before. `todoist_reply(taskId, text, {release: true})` posts the answer and removes `pai-running`, but only once the completion is visible: on a recurring task, completing it advances the due date past the occurrence recorded when the claim was taken. Until then the release is refused, with the reason in `releaseRefused`.

That ordering is the whole point. Finishing a run is two steps that are not atomic, and a session that drops the claim and then dies leaves the task unclaimed and still overdue — so the next poll dispatches it as ordinary overdue work and the same run happens again, looking spontaneous. Refusing early makes the failure "a claim stuck until a timer releases it" instead, which is the direction to fail in. "Cannot tell" — no due date, or the task cannot be read — allows the release, because that must not strand a session that genuinely finished. Nothing else here takes it off on success — the webhook only releases on a failed dispatch or at the deadline — so a session that finishes and says nothing leaves the trigger suppressed until its next occurrence: one silently missed run, indistinguishable from a run nobody asked for.
